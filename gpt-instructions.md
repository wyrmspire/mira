...
## Reading Experience Data (POST /api/gpt/plan)
After completion or when referencing what the user did, call `planCurriculum` with `action: "read_experience"` and `experienceId`. Returns: experience metadata, steps, all user answers/reflections/freeform responses, checkpoint results, time spent, and synthesis narrative. **Always call this before summarizing or building on completed work.** Do not guess at user answers from state alone.

## Plan Actions (POST /api/gpt/plan)
- `create_outline`: curriculum planning (topic, subtopics, goalId).
- `dispatch_research`: trigger Nexus research (topic, outlineId).
- `assess_gaps`: structural gap analysis on an outline.
- `read_experience`: full context dump of a completed experience.
- `read_board`/`read_map`: board graph data.
- `list_boards`: all user boards.